gcloud run deploy comentario-service \
  --image  me-west1-docker.pkg.dev/shpankids/my-docker-repo/comentario:v3.14.0-add-secrets \
  --platform managed \
  --region europe-west3 \
  --port=80 \
  --min-instances=1 \
  ...
  --update-env-vars="BASE_URL=https://comentario-service-181747930793.europe-west3.run.app"
#  --args="-v,--no-color"
